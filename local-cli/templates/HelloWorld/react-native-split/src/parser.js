...
class Parser {
  ...
  splitBundle() {
    ...
    this._generateConfig(outputDir, platform);
  }
  _generateConfig(outputDir, platform) {
    let configJSon = {};
    ...
    collectDependency.forEach(function(dependencyItem, index){
      if(dependencyItem.indexOf('base') > -1){
        ...
        collectCustom.forEach(function(customItem, key){
          if(Util.isExist(customItem.moduleName)){
            return collectCustom.splice(key, 1);
          }
          if(dependencyItem.indexOf(customItem.moduleName) > -1){
            // zzPlaceholder是占位符，到时候需要根据业务线名字进行替换的
            customItem.dependency = `zzPlaceholder/${dependencyItem}|zzPlaceholder/zzPlaceholder-common.${platform}.bundle`
          }
        })
  ...
  _parseAST(bundleAST : any) {
    ...
    body.forEach(node => {
      ...
      if (Util.isPolyfillCall(node, this._config.dev)) { // push polyfill codes to base.
        ...
        console.log('node.expression.type', node.expression.type);
        console.log('node.expression.argument', node.expression.argument);
      }
    });
    ...
    if (!!customEntry) {
      ...
      console.log('customEntrycustomEntrycustomEntry', customEntry);
  ...
  _genBaseModules(moduleId : number) {
    ...
    while(queue.length > 0) {
      ...
      if (this._modules[tmp] && this._modules[tmp].dependencies &&
  ...
  _genCustomEntryModules(name : string, moduleId : string) {
    ...
    while(queue.length > 0) {
      ...
      const dependency = this._modules[tmp] && this._modules[tmp].dependencies;
